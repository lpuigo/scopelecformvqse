...
func TestRecordSet_WriteCSVTo(t *testing.T) {
	...
	if err != nil {
		t.Fatal("TempFile returns ", err.Error())
	}
	tmpfile := f.Name()
	defer os.Remove(f.Name())
	err = cs.WriteCSVTo(f)
	if err != nil {
		t.Fatal("WriteCSVTo returns ", err.Error())
	}
	f.Close()
	f, err = os.Open(tmpfile)
	if err != nil {
		t.Fatal("can not open temp file ", err.Error())
	}
	defer f.Close()
	...
	if cs2.Len() != 2 {
		t.Fatalf("cloned RecordSet has unexpected length %d (expected %d)", cs2.Len(), cs.Len())
	}
